(function () {
   ...
   * download path so its own Image.Create.Post hook picks it up automatically.
   */
  async function saveImageToGallery(performerId, imageUrl, source) {
    try {
      const gqlQuery = `
        mutation RunPluginOperation($plugin_id: ID!, $args: Map) {
          runPluginOperation(plugin_id: $plugin_id, args: $args)
      ...
      const output = data?.runPluginOperation;
      if (!output) {
        throw new Error("No response from save-to-gallery operation");
      }
      if (output.error) {
        throw new Error(output.error);
      }
      ...
      console.error("[PerformerImageSearch] Failed to save to gallery:", e);
      throw e;
    }
  }
  ...
  /**
   * Confirm and save the previewed image into the shared performer gallery
   * (Performer Gallery Sync folder/tag/gallery system) instead of setting it
   * as the single performer profile image.
   */
  window.pisSaveToGallery = async function () {
    if (!previewImage || !currentPerformerId) return;

    const galleryBtn = document.getElementById("pis-gallery-btn");
    const originalText = galleryBtn?.textContent;

    if (galleryBtn) {
      galleryBtn.disabled = true;
      galleryBtn.textContent = "Saving...";
      galleryBtn.classList.add("pis-btn-loading");
    }

    showStatus("Saving image to gallery...", "loading");

    try {
      await saveImageToGallery(currentPerformerId, previewImage, previewSource);
      showStatus("Saved to gallery!", "success");

      if (galleryBtn) {
        galleryBtn.textContent = "Saved!";
      }

      setTimeout(() => {
        if (galleryBtn) {
          galleryBtn.disabled = false;
          galleryBtn.textContent = originalText;
          galleryBtn.classList.remove("pis-btn-loading");
        }
      }, 1500);
    } catch (e) {
      showStatus(`Failed to save to gallery: ${e.message}`, "error");
      if (galleryBtn) {
        galleryBtn.disabled = false;
        galleryBtn.textContent = originalText;
        galleryBtn.classList.remove("pis-btn-loading");
