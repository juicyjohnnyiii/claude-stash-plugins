...
def processQueue():
    ...
    if (
        stash.get_configuration()["plugins"]["performerGallerySync"]
        == settings["queue"]
    ):
        ...
        stash.run_plugin_task(
            "performerGallerySync", "Process Performers", args={"full": False}
        )
...
if "performerGallerySync" in config:
    settings.update(config["performerGallerySync"])
# log.info('config: %s ' % (settings,))


tag_stashbox_performer_gallery = stash.find_tag(
    "[Performer Gallery Sync]", create=True
).get("id")
tag_performer_image = stash.find_tag("[Set Profile Pic]", create=True).get("id")
